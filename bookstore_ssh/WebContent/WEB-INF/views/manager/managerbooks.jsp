		...
		$(".edit").click(
				function() {
					...
					$.get(url, args, function(book) {
						...
						$(".editform").find("input[name=title]")
								.val(book.title);
						$(".editform").find("input[name=author]").val(
								book.author);
						$(".editform").find("input[name=storeNumber]").val(
								book.storeNumber);
						$(".editform").find("input[name=price]")
								.val(book.price);
						$("#myModal").modal("show");
					}, "json")
					return false;
	...
	<div class="container-fluid bookmanager">
		<div class="row">
			<div class="col-xs-12 col-md-3">
				<form
					action="<%=request.getContextPath()%>/manager/doaddbook?pagenumber=${page.pageNo }"
					...
					<button type="submit"
						class="btn btn-success glyphicon glyphicon-ok">添加图书</button>
				</form>
			</div>
				...
				<table class="table table-condensed table-hover"
					...
					<c:forEach items="${page.list }" var="book">
						<tr>
							...
							<td>${book.remark }</td>
							<td><a
								href="<%=request.getContextPath() %>/manager/dodeletebook"
								class="btn btn-danger btn-sm delete"
								bookid="${book.id }" booktitle=${book.title }
								pagenumber="${page.pageNo }"><span class="fui-trash"></span>删除</a></td>
							<td><a
								href="<%=request.getContextPath() %>/manager/editbook"
